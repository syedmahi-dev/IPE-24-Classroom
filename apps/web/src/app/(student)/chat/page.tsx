...
export default function ChatPage() {
  ...
  return (
    <div className="max-w-4xl mx-auto h-[calc(100vh-140px)] flex flex-col pb-6">
      <div className="glass rounded-[2rem] flex flex-col h-full overflow-hidden shadow-2xl shadow-indigo-900/5 relative">
        <div className="absolute inset-0 bg-gradient-to-b from-indigo-50/50 to-white/20 pointer-events-none" />
        
        {/* Chat Header */}
        <div className="p-6 md:p-8 flex items-center justify-between border-b border-white/60 dark:border-white/10 bg-white/40 dark:bg-slate-900/40 z-10 backdrop-blur-md">
          <div className="flex items-center gap-5">
            <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center text-white shadow-lg shadow-indigo-500/30 relative">
               <Brain className="w-7 h-7" />
               <div className="absolute -bottom-1 -right-1 w-4 h-4 bg-emerald-500 border-2 border-white dark:border-slate-800 rounded-full animate-pulse" />
            </div>
            <div>
              <h2 className="text-xl md:text-2xl font-black text-slate-800 dark:text-slate-100">IPE-24 Virtual CR</h2>
              <p className="text-sm font-bold text-indigo-600 dark:text-indigo-400">Gemini 1.5 Powered Assistant</p>
            </div>
          </div>
        </div>

        {/* Messages */}
        <div className="flex-1 overflow-y-auto p-6 md:p-8 space-y-8 z-10 scrollbar-hide">
          {historyLoading ? (
            ...
            messages.map((m, i) => (
              <div key={i} className={`flex gap-4 ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                ...
                <div className={`max-w-[80%] rounded-[1.5rem] px-6 py-4 font-medium text-[15px] leading-relaxed shadow-sm ${
                  m.role === 'user' 
        ...
        {/* Input */}
        <div className="p-6 md:p-8 bg-white/60 dark:bg-slate-900/60 border-t border-white/60 dark:border-white/10 z-10 backdrop-blur-md">
          <form onSubmit={handleSubmit} className="relative flex items-center group">
            <input
              ...
              className="w-full pl-6 pr-16 py-5 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-[1.5rem] shadow-sm focus:outline-none focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-400 text-slate-700 dark:text-slate-200 font-bold transition-all placeholder:font-medium placeholder:text-slate-400 dark:placeholder:text-slate-500"
            />
            <button
              type="submit"
              disabled={isLoading || !input.trim()}
              className="absolute right-3 p-3 bg-indigo-600 text-white rounded-xl hover:bg-indigo-700 disabled:bg-slate-200 disabled:text-slate-400 transition-all hover:scale-105 active:scale-95 flex items-center justify-center"
            >
